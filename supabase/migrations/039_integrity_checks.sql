-- =====================================================
-- MIGRACIÓN 039: Verificación de Integridad Periódica
-- =====================================================
-- Job que detecta inconsistencias en el sistema de billing

-- 1. Tabla para almacenar resultados de verificaciones de integridad
...
-- 2. Índices
CREATE INDEX IF NOT EXISTS idx_integrity_check_type ON integrity_check_results(check_type);
CREATE INDEX IF NOT EXISTS idx_integrity_check_status ON integrity_check_results(status);
CREATE INDEX IF NOT EXISTS idx_integrity_check_date ON integrity_check_results(checked_at DESC);

-- 3. Función para verificar suscripciones ACTIVE sin preapproval
...
-- 4. Función para verificar múltiples trials por usuario
...
-- 5. Función para verificar extensiones de trial excesivas
...
-- 6. Función para verificar inconsistencias en usage_metrics
...
-- 7. Función master que ejecuta todas las verificaciones
CREATE OR REPLACE FUNCTION run_all_integrity_checks()
RETURNS JSONB AS $$
DECLARE
  results JSONB := '{}'::JSONB;
BEGIN
  -- Ejecutar todas las verificaciones
  results := results || jsonb_build_object('active_without_preapproval', check_active_without_preapproval());
  results := results || jsonb_build_object('multiple_trials_per_user', check_multiple_trials_per_user());
  results := results || jsonb_build_object('excessive_trial_extensions', check_excessive_trial_extensions());
  results := results || jsonb_build_object('usage_metrics_integrity', check_usage_metrics_integrity());
  
  -- Agregar timestamp
  results := results || jsonb_build_object('checked_at', NOW());
...
-- 8. Comentarios
COMMENT ON TABLE integrity_check_results IS 'Resultados de verificaciones de integridad periódicas';
COMMENT ON FUNCTION check_active_without_preapproval() IS 'Verifica suscripciones ACTIVE sin preapproval válido';
COMMENT ON FUNCTION check_multiple_trials_per_user() IS 'Verifica usuarios con múltiples trials';
COMMENT ON FUNCTION check_excessive_trial_extensions() IS 'Verifica extensiones de trial excesivas';
COMMENT ON FUNCTION check_usage_metrics_integrity() IS 'Verifica integridad de usage_metrics';
COMMENT ON FUNCTION run_all_integrity_checks() IS 'Ejecuta todas las verificaciones de integridad';
