...
import { Loader2, Plus, FileText, Send, Eye, Download, Search, Filter, ShieldCheck, CheckCircle2, XCircle } from "lucide-react"
import {
  Breadcrumb,
  BreadcrumbList,
  BreadcrumbItem,
...
export function InvoicesPageClient() {
  ...
  const verifyInAfip = async (invoice: Invoice) => {
    if (!invoice.cbte_nro || !invoice.pto_vta || !invoice.cbte_tipo) {
      toast({
        title: "No se puede verificar",
        description: "La factura no tiene datos de comprobante completos",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast({
          ...
          description: data.error || "No se pudo consultar AFIP",
          variant: "destructive",
        })
        return
      }
      ...
      if (data.exists) {
        toast({
          ...
          description: `CAE ${data.voucherInfo.CAE} confirmado en AFIP`,
        })
      } else {
        toast({
          title: "No encontrado",
          description: data.message || "El comprobante no fue encontrado en AFIP",
          variant: "destructive",
        })
      }
    } catch (error: any) {
      ...
      toast({
        title: "Error",
        description: error.message || "Error al verificar en AFIP",
        variant: "destructive",
      })
    } finally {
      setVerifying(false)
                    ...
                    <div className="rounded-lg border border-green-200 bg-green-50 dark:bg-green-950/20 dark:border-green-800 p-4 space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>
                          ...
                          <span className="font-mono">{verifyResult.voucherInfo.CAE}</span>
                        </div>
                        <div>
